...
class FighterDefenseTokenTest {
	...
	@Test
	@DisplayName("Testing relationship between FighterDefenseToken and DefenseToken")
	void testDefenseTokenRelationship() {
		assertNotNull(fighterToken.getDefenseToken());
		assertTrue(fighterToken.getDefenseToken().getName().equals("Brace"));
		assertTrue(fighterToken.getDefenseToken().getEffect().equals("After damage is totaled, the defender reduces the total by half, rounded up."));
	}
